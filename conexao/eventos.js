...
export const EventosCliente = {
    ...
    CRIAR_SALA: 'criarSala',    // { numberPlayers, roundStart, randomShuffle, botNumber, modeloBot, chatAberto, privada } -> ack: { ok, salaId, numberPlayers, jogadores, segundosParaIniciar, chatAberto, modeloBot, senha } — botNumber preenche o resto dos assentos com bots (ver bots/Bot.js); modeloBot (default 'classico') escolhe qual bot joga por eles e pelo automático de quem cair (ids em bots/modelosBot.js); segundosParaIniciar != null se os bots já lotaram a sala; chatAberto (default false) libera o chat de texto livre da sala. privada (default false): sala continua aparecendo em listarSalas normalmente, mas entrarSala exige senha batendo. O CRIADOR não escolhe a senha — o servidor sorteia 4 dígitos e devolve em `senha` só neste ack (null se a sala não é privada); é o criador quem repassa pra quem quiser convidar, nenhum outro ack/broadcast expõe a senha de novo. Erros de teto: LIMITE_DE_SALAS (global), LIMITE_DE_SALAS_POR_JOGADOR (por pessoa), JA_EM_PARTIDA (já tem assento numa partida em andamento)
    ENTRAR_SALA: 'entrarSala',  // { salaId, senha? } -> ack: { ok, salaId, numberPlayers, jogadores, segundosParaIniciar, chatAberto, modeloBot } — segundosParaIniciar != null se esta entrada lotou a sala. `senha` só é conferida (e obrigatória de bater) se a sala é privada — ver LISTAR_SALAS; SENHA_INCORRETA se não bater
    PARTIDA_RAPIDA: 'partidaRapida', // {} -> ack: { ok, salaId, numberPlayers, jogadores, segundosParaIniciar, chatAberto, modeloBot } — mesmo formato de criarSala/entrarSala; entra numa fila compartilhada de sala default (config igual criarSala sem parâmetros, logo nunca privada), criando-a se não houver nenhuma aberta no momento
    LISTAR_SALAS: 'listarSalas', // {} -> ack: { ok, salas: [{ salaId, numberPlayers, jogadoresAtual, chatAberto, privada, modeloBot }] } — privada só indica que entrarSala vai pedir senha; a senha em si nunca é exposta aqui
    ...
    JOGAR_DE_NOVO: 'jogarDeNovo', // { salaId } -> ack: { ok, salaId, numberPlayers, jogadores, segundosParaIniciar, chatAberto, modeloBot } — só o adm da sala TERMINADA (`salaId` é a sala antiga); cria uma sala nova com a mesma config e avisa quem mais estava lá (ver EventosServidor.CONVITE_REVANCHE)
    ...
    SUGESTAO_BOT: 'sugestaoBot', // { salaId } -> ack: { ok, tipo: 'aposta', valor, modeloBot } | { ok, tipo: 'carta', indice, carta, modeloBot } — "dica do bot": o que o bot da sala (modeloBot) faria no seu lugar agora, sem jogar nada. Só na sua vez (de apostar ou de jogar); fora dela NAO_E_SUA_VEZ
